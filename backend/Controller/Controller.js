...
const home=async (req,res)=>{
    ...
    const user=new User({Username:"madanlal",Email:"madanlal@gmail.com",Password:"0000",Image:"Image.png"})
    try
    {
        await user.save()
    }
...
const Update=async (req,res)=>{
    ...
    await User.updateOne({_id:id},{Username:Username,Email:Email,Password:Password}).then(function(){
    ...
    User.findOne({Username:Username},(err,user)=>{
        if(user)
        {
            bcrypt.compare(Password,user.Password,(err,validate)=>{
                if(validate)
                {
                    const token=jwt.sign({Username:user.Username},"MYSECRETKEY",{expiresIn:30*60});
                    res.json(token)
                }
                else
                {
